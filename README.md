...
- **See real capability badges** — tool calling, reasoning, vision, audio, long context, free tier — all derived from the API response, not guessed. "Image Output" (pixel image generation) is shown but deliberately never scored for diagram tasks: today's models don't reliably produce standards-compliant, tag-numbered P&IDs, so what's actually useful for generating one is reasoning + structured/parseable output, not image generation.
